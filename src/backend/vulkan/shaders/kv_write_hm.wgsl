...
// Model dims come from the uniform (n = kv_dim, hd) so one SPV serves any hd<=64
// model; MAX_SEQ is the fixed engine cache cap (mod.rs:1156), identical for all.
struct P { n: u32, base: u32, hd: u32, p1: u32 };
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    let i = gid.x;
    if (i >= p.n) { return; }
    let pos = p.base / p.n;   // n = kv_dim
    let kvh = i / p.hd;
    let d = i % p.hd;
    dst[kvh * MAX_SEQ * p.hd + pos * p.hd + d] = src[i];
}
